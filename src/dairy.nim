proc decideDairy(controller: Controller, env: Environment, agent: Thing,
                agentId: int, state: var AgentState): uint8 =
  # Priority 1: If carrying bread, deliver to teammates needing it.
  ...
  # Priority 2: If we have milk and wheat, bake enriched bread.
  if getInv(agent, ItemMilk) > 0 and agent.inventoryWheat > 0:
    let (did, act) = controller.findAndUseBuilding(env, agent, agentId, state, ClayOven)
    if did: return act

  # Priority 3: Collect wheat if we have milk but no grain.
  if getInv(agent, ItemMilk) > 0 and agent.inventoryWheat == 0:
    let (did, act) = controller.findAndHarvest(env, agent, agentId, state, Wheat)
    if did: return act

  # Priority 4: Milk cows.
  let (did, act) = controller.findAndUseBuilding(env, agent, agentId, state, Cow)
  if did: return act
