...
proc nearestFriendlyBuilding(env: Environment, state: var AgentState, teamId: int,
                             kind: ThingKind, requireReady = false): Thing {.inline.} =
  let building = env.findNearestFriendlyThingSpiral(state, teamId, kind)
  if requireReady and (isNil(building) or building.cooldown != 0):
    return nil
  building
...
proc useNearestFriendlyBuilding(controller: Controller, env: Environment, agent: Thing,
                                ...
                                kind: ThingKind, requireReady = false): uint16 {.inline.} =
  let building = env.nearestFriendlyBuilding(state, teamId, kind, requireReady)
  if isNil(building):
    return 0'u16
...
proc optCraftBread*(controller: Controller, env: Environment, agent: Thing,
                    agentId: int, state: var AgentState): uint16 =
  let teamId = getTeamId(agent)
  controller.useNearestFriendlyBuilding(env, agent, agentId, state, teamId, ClayOven, true)
...
proc optSiegeBreacher(controller: Controller, env: Environment, agent: Thing,
                      agentId: int, state: var AgentState): uint16 =
  let teamId = getTeamId(agent)
  controller.useNearestFriendlyBuilding(env, agent, agentId, state, teamId, SiegeWorkshop, true)
...
proc optMangonelSuppression(controller: Controller, env: Environment, agent: Thing,
                            agentId: int, state: var AgentState): uint16 =
  let teamId = getTeamId(agent)
  controller.useNearestFriendlyBuilding(env, agent, agentId, state, teamId, MangonelWorkshop, true)
...
proc optMarketTrade*(controller: Controller, env: Environment, agent: Thing,
  ...
  let teamId = getTeamId(agent)
  state.basePosition = agent.getBasePos()
  controller.useNearestFriendlyBuilding(env, agent, agentId, state, teamId, Market, true)
...
proc optResearchUniversityTech*(controller: Controller, env: Environment, agent: Thing,
                                agentId: int, state: var AgentState): uint16 =
  let teamId = getTeamId(agent)
  controller.useNearestFriendlyBuilding(env, agent, agentId, state, teamId, University, true)
...
proc optResearchEconomyTech*(controller: Controller, env: Environment, agent: Thing,
  ...
  if not found:
    return 0'u16
  controller.useNearestFriendlyBuilding(env, agent, agentId, state, teamId, buildingKind, true)
...
proc optResearchBlacksmithUpgrade*(controller: Controller, env: Environment, agent: Thing,
                                    agentId: int, state: var AgentState): uint16 =
  let teamId = getTeamId(agent)
  controller.useNearestFriendlyBuilding(env, agent, agentId, state, teamId, Blacksmith, true)
...
proc optResearchUnitUpgrade*(controller: Controller, env: Environment, agent: Thing,
  ...
  for kind in UnitUpgradeBuildings:
    ...
    if not env.canSpendStockpile(teamId, costs):
      continue
    let building = env.nearestFriendlyBuilding(state, teamId, kind, true)
    if isNil(building):
      continue
