...
proc findNearestEnemyBuilding(env: Environment, pos: IVec2, teamId: int): Thing =
  ...
  const EnemyBuildingKinds = [
    TownCenter, House, Door, ClayOven, WeavingLoom, Outpost, GuardTower, Barrel,
  ...
  for kind in EnemyBuildingKinds:
...
proc findNearestEnemyPresence(env: Environment, pos: IVec2,
  ...
  ## Find nearest enemy presence (agent or building).
  ## Optimized: iterates only building kinds using thingsByKind instead of all things.
  const EnemyBuildingKinds = [
    TownCenter, House, Door, ClayOven, WeavingLoom, Outpost, GuardTower, Barrel,
    Mill, Granary, LumberCamp, Quarry, MiningCamp,
...
proc findNearestNeutralHub(env: Environment, pos: IVec2): Thing =
  ## Find nearest neutral hub building.
  ## Optimized: iterates only hub kinds using thingsByKind instead of all things.
  const NeutralHubKinds = [
    Castle, Market, Outpost, University, Blacksmith, Barracks,
...
proc optLanternGapFill(controller: Controller, env: Environment, agent: Thing,
                       agentId: int, state: var AgentState): uint8 =
  ## Find buildings without lantern coverage and place a lantern nearby.
  ## Optimized: iterates only building kinds using thingsByKind instead of all things.
