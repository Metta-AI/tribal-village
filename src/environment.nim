...
proc writeTileObs(env: Environment, agentId, obsX, obsY, worldX, worldY: int) {.inline.} =
  ...
  ## Optimized: single nil check per grid, no intermediate variables, direct writes.
  var agentObs = addr env.observations[agentId]

  # Always-written layers (terrain one-hot, biome value)
  ...
  # Background thing layer
  if not isNil(backgroundThing):
    agentObs[][ThingLayerStart + ord(backgroundThing.kind)][obsX][obsY] = 1

  # Single branching structure for blocking thing (avoids repeated nil checks)
  if isNil(blockingThing):
    # No blocking thing - check team from background only
    if not isNil(backgroundThing) and backgroundThing.kind in TeamOwnedKinds and
      ...
      agentObs[][ord(TeamLayer)][obsX][obsY] = (backgroundThing.teamId + 1).uint8
  else:
    # Have blocking thing - write its layer
    agentObs[][ThingLayerStart + ord(blockingThing.kind)][obsX][obsY] = 1

    if blockingThing.kind == Agent:
      # Agent-specific layers - write directly (no intermediate vars)
      ...
      if blockingThing.unitClass == UnitTrebuchet and blockingThing.packed:
        agentObs[][ord(TrebuchetPackedLayer)][obsX][obsY] = 1
    else:
      # Non-agent blocking thing (building/resource)
      # Team ownership: check blocking first, then background
      if blockingThing.kind in TeamOwnedKinds and
         blockingThing.teamId >= 0 and blockingThing.teamId < MapRoomObjectsTeams:
        agentObs[][ord(TeamLayer)][obsX][obsY] = (blockingThing.teamId + 1).uint8
...
proc rebuildObservations*(env: Environment) =
  ...
  ## Optimized: pre-compute valid observation bounds to avoid per-tile bounds checks.
  ...
  for agentId in 0 ..< env.agents.len:
    ...
    # Pre-compute valid observation bounds (avoid per-tile bounds checks)
    # obsX maps to worldX = agentPos.x + (obsX - ObservationRadius)
    # Valid when 0 <= worldX < MapWidth
    # => 0 <= agentPos.x + obsX - ObservationRadius < MapWidth
    ...
    # Skip agents with no valid observation area (shouldn't happen normally)
    if obsXStart >= obsXEnd or obsYStart >= obsYEnd:
      continue

    # Pre-compute world coordinate base offset
    let worldXBase = agentPos.x - ObservationRadius
    let worldYBase = agentPos.y - ObservationRadius

    for obsX in obsXStart ..< obsXEnd:
      let worldX = worldXBase + obsX
      for obsY in obsYStart ..< obsYEnd:
        let worldY = worldYBase + obsY
